...
`#flagCard` has its own subtitle, `#subtitle`, reading "Use the colour picker below to fill in the flag's missing section" — shown above the picker row. `#result` has a separate one, `#subtitleResult`, reading "Come back tomorrow for a new flag!" — shown below the score feedback. Both use the `.subtitle` class.
