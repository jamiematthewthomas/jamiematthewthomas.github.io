...
- `#flagName` lives inside `#flagCard`, between `#flagSvg` and `#pickerRow`, so it's hidden along with the rest of the picker card once a guess has been made — `#flagNameResult` (inside `#result`) shows the same name afterwards.
